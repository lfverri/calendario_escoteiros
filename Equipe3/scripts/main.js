...
let nav = 0;
let clicked = null;
let events = localStorage.getItem('events_3') ? JSON.parse(localStorage.getItem('events_3')) : [];
    ...
    for (let i = 1; i <= paddinDays + daysMonth; i++) {
        ...
        if (i > paddinDays) {
            ...
            if (eventDay) {
                ...
                eventDiv.classList.add('event_2');
...
passwordSubmitButton.addEventListener('click', function () {
    if (passwordInput.value === correctPassword) {
        if (eventTitleInput.value) {
            ...
            localStorage.setItem('events_2', JSON.stringify(events));
