...
let nav = 0;
let clicked = null;
let events = localStorage.getItem("events_1")
  ? JSON.parse(localStorage.getItem("events_1"))
  : [];
...
function load() {
  ...
  for (let i = 1; i <= paddinDays + daysMonth; i++) {
    ...
    const dayString = `${String(i - paddinDays).padStart(2, "0")}/${String(
      month + 1
    ).padStart(2, "0")}/${year}`;
  ...
  const teamEvents = events.filter((event) => event.team === currentTeam);
}
    ...
    if (passwordInput.value === correctPassword) {
      if (eventTitleInput.value) {
        const newEvent = {
          ...
          team: "Patrulha1", // Adicione a equipe correspondente aqui
        };

        events.push(newEvent);
        localStorage.setItem("events_1", JSON.stringify(events));
...
passwordSubmitButton.addEventListener("click", function () {
  if (passwordInput.value === correctPassword) {
    if (eventTitleInput.value) {
      ...
      events.push(newEvent);
      localStorage.setItem(currentPage + "events_1", JSON.stringify(events));
